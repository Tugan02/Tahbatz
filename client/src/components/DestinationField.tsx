...
type Props = {
  ...
  onPickDestination: (lat: number, lng: number, label: string) => void
}

export default function DestinationField({ originLat, originLng, value, onChange, onPickDestination }: Props) {
  const [suggestions, setSuggestions] = useState<any[]>([])
  const [loading, setLoading] = useState(false)
  const abortRef = useRef<AbortController | null>(null)
  const timerRef = useRef<number | null>(null)

  // עוטף את לוגיקת autosuggest
  const handleChange = (q: string) => {
    onChange(q)
    setSuggestions([])
    // lat,lon → אין הצעות; אפשר להמיר ישירות מאוחר יותר
    ...
    timerRef.current = window.setTimeout(async () => {
      try {
        ...
        setLoading(true)
        const at = (originLat && originLng) ? `${originLat},${originLng}` : `32.0853,34.7818`
  ...
  const onPick = (label: string, lat?: number, lng?: number) => {
    if (lat != null && lng != null) {
      ...
      // נפילה אחורית – השלמה דרך geocode מלא
      apiGeocode(label).then(d => {
        const p = d.items?.[0]?.position
        if (p) onPickDestination(p.lat, p.lng, label)
      })
    }
    setSuggestions([])
  }
